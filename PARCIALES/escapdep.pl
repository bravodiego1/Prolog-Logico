...
sala(miseriaDeLaNoche,terrorifica(150,22)). % PUNTO 6
...
% 1) nivelDeDificultadDeLaSala/2: para cada sala nos dice su dificultad. Para las salas de experiencia terrorífica el nivel de dificultad 
% es la cantidad de sustos menos la edad mínima para ingresar. Para las de experiencia familiar es 15 si es de una temática futurista y
% para cualquier otra temática es la cantidad de habitaciones. El de las enigmáticas es la cantidad de candados que tenga.
...
dificultadSegunExperiencia(familiar(Tematica,CantidadDeHabitaciones),CantidadDeHabitaciones):-
    Tematica \= futurista.

dificultadSegunExperiencia(enigmatica(Candados),CantidadDeCandados):-
    length(Candados,CantidadDeCandados).

% 2) puedeSalir/2: una persona puede salir de una sala si la dificultad de la sala es 1 o si tiene más de 13 años y la dificultad es menor a 5.
% En ambos casos la persona no debe ser claustrofóbica.

puedeSalir(Persona,Sala):-
    persona(Persona,Edad,Peculiaridades),
    sala(Sala,Experiencia),
    cumpleCondicionesDeSalida(Experiencia,Edad),
    not(esClaustrofobicaPersona(Peculiaridades)).

cumpleCondicionesDeSalida(Experiencia,_):-
    dificultadSegunExperiencia(Experiencia,1).

cumpleCondicionesDeSalida(Experiencia,Edad):-
    Edad > 13,
    dificultadSegunExperiencia(Experiencia,Dificultad),
    between(0,4,Dificultad).

esClaustrofobicaPersona(Peculiaridades):-
...
empresaCopada(Empresa):-
    ...
    between(0,3,Promedio)

promedioDeDificultadDeSalas(Empresa,Total):-
    esSalaDe(_,Empresa),
    findall(Dificultad,(esSalaDe(Sala,Empresa),nivelDeDificultadDeLaSala(Sala,Dificultad)),DificultadDeSalasDeLaEmpresa),
    promedio(DificultadDeSalasDeLaEmpresa,Total).
...
% La empresa supercelula es dueña de salas de escape familiares ambientadas en videojuegos. La sala estrellasDePelea cuenta con 
% 7 habitaciones pero lamentablemente no sabemos la cantidad que tiene su nueva sala choqueDeLaRealeza.
% La empresa SKPista (fanática de un famoso escritor) es la dueña de una única sala terrorífica para mayores de 21 llamada miseriaDeLaNoche
% que nos asegura 150 sustos.
